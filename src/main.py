...
def get_contributions_data():
    ...
    query = f"""
    {{
        user(login: "{os.getenv("GITHUB_USERNAME")}") {{
...
def main():
    ...
    dates = [datetime.datetime.strptime(date, "%Y-%m-%d") for date in contribution_dates]
    streak = 0
    ...
    message = (
                ...
                f"🧑‍💻 GitHub Username: {os.getenv('GITHUB_USERNAME')}\n\n"
